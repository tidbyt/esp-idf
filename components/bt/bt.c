...
#define BTDM_MODEM_SLEEP_MODE_EVED \
    (2)  // sleep mode for BLE controller, used only for internal test.
...
#define BTDM_MIN_SLEEP_DURATION \
    (12)  // threshold of interval in slots to allow to fall into modem sleep
#define BTDM_MODEM_WAKE_UP_DELAY \
    (4)  // delay in slots of modem wake up procedure, including re-enable
         // PHY/RF
...
typedef struct vhci_host_callback {
    void (*notify_host_send_available)(
        void); /*!< callback used to notify that the host can send packet to
                  controller */
    int (*notify_host_recv)(
        uint8_t *data,
        uint16_t len); /*!< callback used to notify that the controller has a
                          packet to send to the host*/
...
struct osi_funcs_t {
    ...
    void (*_btdm_sleep_enter_phase1)(
        uint32_t lpcycles); /* called when interrupt is disabled */
    void (*_btdm_sleep_enter_phase2)(void);
    void (*_btdm_sleep_exit_phase1)(void); /* called from ISR */
    void (*_btdm_sleep_exit_phase2)(void); /* called from ISR */
    void (*_btdm_sleep_exit_phase3)(void); /* called from task */
    int (*_coex_bt_request)(uint32_t event, uint32_t latency,
                            uint32_t duration);
...
extern void btdm_rf_bb_init_phase2(
    void);  // shall be called after PHY/RF is enabled
/* Sleep */
...
/* Coexistence */
extern int coex_bt_request_wrapper(uint32_t event, uint32_t latency,
                                   uint32_t duration);
...
static int32_t IRAM_ATTR queue_send_from_isr_wrapper(void *queue, void *item,
                                                     void *hptw);
static int32_t queue_recv_wrapper(void *queue, void *item,
                                  uint32_t block_time_ms);
static int32_t IRAM_ATTR queue_recv_from_isr_wrapper(void *queue, void *item,
                                                     void *hptw);
...
static btdm_dram_available_region_t btdm_dram_available_region[] = {
    ...
    {ESP_BT_MODE_CLASSIC_BT, SOC_MEM_BT_EM_BREDR_START,
...
SOC_RESERVE_MEMORY_REGION(SOC_MEM_BT_EM_START, SOC_MEM_BT_EM_BREDR_REAL_END,
                          rom_bt_em);
SOC_RESERVE_MEMORY_REGION(SOC_MEM_BT_BSS_START, SOC_MEM_BT_BSS_END, rom_bt_bss);
SOC_RESERVE_MEMORY_REGION(SOC_MEM_BT_MISC_START, SOC_MEM_BT_MISC_END,
                          rom_bt_misc);
SOC_RESERVE_MEMORY_REGION(SOC_MEM_BT_DATA_START, SOC_MEM_BT_DATA_END,
                          rom_bt_data);
...
static DRAM_ATTR uint8_t btdm_lpcycle_us_frac =
    0;  // number of fractional bit for btdm_lpcycle_us

#ifdef CONFIG_PM_ENABLE
static DRAM_ATTR esp_timer_handle_t s_btdm_slp_tmr;
static DRAM_ATTR esp_pm_lock_handle_t s_pm_lock;
static DRAM_ATTR esp_pm_lock_handle_t
    s_light_sleep_pm_lock;  // pm_lock to prevent light sleep due to
...
static void IRAM_ATTR task_yield_from_isr(void) { portYIELD_FROM_ISR(); }
...
static void *semphr_create_wrapper(uint32_t max, uint32_t init) {
    ...
    queue_buffer = heap_caps_malloc(sizeof(StaticQueue_t),
...
static int32_t semphr_take_wrapper(void *semphr, uint32_t block_time_ms) {
    if (block_time_ms == OSI_FUNCS_TIME_BLOCKING) {
        ...
        return (int32_t)xSemaphoreTake(semphr,
                                       block_time_ms / portTICK_PERIOD_MS);
    }
}
...
static void *queue_create_wrapper(uint32_t queue_len, uint32_t item_size) {
    ...
    queue_storage = heap_caps_malloc((queue_len * item_size),
                                     MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (!queue_storage) {
        goto error;
    }
    ...
    handle =
        xQueueCreateStatic(queue_len, item_size, queue_storage, queue_buffer);
    if (!handle) {
        goto error;
    }
...
static int32_t queue_send_wrapper(void *queue, void *item,
    ...
    if (block_time_ms == OSI_FUNCS_TIME_BLOCKING) {
        ...
        return (int32_t)xQueueSend(queue, item,
                                   block_time_ms / portTICK_PERIOD_MS);
    }
}
...
static int32_t queue_recv_wrapper(void *queue, void *item,
    ...
    if (block_time_ms == OSI_FUNCS_TIME_BLOCKING) {
        ...
        return (int32_t)xQueueReceive(queue, item,
                                      block_time_ms / portTICK_PERIOD_MS);
    }
}
...
static void task_delete_wrapper(void *task_handle) { vTaskDelete(task_handle); }
...
static void *malloc_internal_wrapper(size_t size) {
    return heap_caps_malloc(
        size, MALLOC_CAP_8BIT | MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
}
...
static void IRAM_ATTR srand_wrapper(unsigned int seed) { /* empty function */ }

static int IRAM_ATTR rand_wrapper(void) { return (int)esp_random(); }
...
static uint32_t IRAM_ATTR btdm_us_2_lpcycles(uint32_t us) {
    ...
    uint64_t cycles =
        ((uint64_t)(us) << btdm_lpcycle_us_frac) / btdm_lpcycle_us;
...
static void btdm_sleep_enter_phase1_wrapper(uint32_t lpcycles) {
#ifdef CONFIG_PM_ENABLE
    // start a timer to wake up and acquire the pm_lock before modem_sleep
    // awakes
    uint32_t us_to_sleep = btdm_lpcycles_2_us(lpcycles);

#define BTDM_MIN_TIMER_UNCERTAINTY_US (1800)
    assert(us_to_sleep > BTDM_MIN_TIMER_UNCERTAINTY_US);
    // allow a maximum time uncertainty to be about 488ppm(1/2048) at least as
    // clock drift and set the timer in advance
    uint32_t uncertainty = (us_to_sleep >> 11);
    if (uncertainty < BTDM_MIN_TIMER_UNCERTAINTY_US) {
        uncertainty = BTDM_MIN_TIMER_UNCERTAINTY_US;
    }

    if (esp_timer_start_once(s_btdm_slp_tmr, us_to_sleep - uncertainty) !=
        ESP_OK) {
...
esp_err_t esp_vhci_host_register_callback(
    const esp_vhci_host_callback_t *callback) {
    return API_vhci_host_register_callback(
               (const vhci_host_callback_t *)callback) == 0
               ? ESP_OK
               : ESP_FAIL;
}
...
static void btdm_controller_mem_init(void) {
    ...
    for (int i = 1; i < sizeof(btdm_dram_available_region) /
...
static esp_err_t try_heap_caps_add_region(intptr_t start, intptr_t end) {
    ...
    /* heap_caps_add_region() returns ESP_ERR_INVALID_SIZE if the memory region
     * is is too small to fit a heap. This cannot be termed as a fatal error and
     * hence we replace it by ESP_OK
     */
...
esp_err_t esp_bt_controller_mem_release(esp_bt_mode_t mode) {
    ...
    for (int i = 0; i < sizeof(btdm_dram_available_region) /
                            sizeof(btdm_dram_available_region_t);
         i++) {
        // skip the share mode, idle mode and other mode
        if (btdm_dram_available_region[i].mode == ESP_BT_MODE_IDLE ||
            (mode & btdm_dram_available_region[i].mode) !=
                btdm_dram_available_region[i].mode) {
        ...
        if (i < sizeof(btdm_dram_available_region) /
                        sizeof(btdm_dram_available_region_t) -
                    1) {
            mem_end = btdm_dram_available_region[i].end;
            if (btdm_dram_available_region[i + 1].mode != ESP_BT_MODE_IDLE &&
                (mode & btdm_dram_available_region[i + 1].mode) ==
                    btdm_dram_available_region[i + 1].mode &&
...
esp_err_t esp_bt_controller_init(esp_bt_controller_config_t *cfg) {
    ...
    osi_funcs_p = (struct osi_funcs_t *)malloc_internal_wrapper(
        sizeof(struct osi_funcs_t));
    if (osi_funcs_p == NULL) {
...
#ifdef CONFIG_PM_ENABLE
    if ((err = esp_pm_lock_create(ESP_PM_NO_LIGHT_SLEEP, 0, "btLS",
                                  &s_light_sleep_pm_lock)) != ESP_OK) {
        goto error;
    }
    if ((err = esp_pm_lock_create(ESP_PM_APB_FREQ_MAX, 0, "bt", &s_pm_lock)) !=
        ESP_OK) {
        goto error;
    }
    esp_timer_create_args_t create_args = {
        .callback = btdm_slp_tmr_callback, .arg = NULL, .name = "btSlp"};
...
esp_err_t esp_bt_controller_enable(esp_bt_mode_t mode) {
    ...
    if (btdm_controller_get_sleep_mode() == BTDM_MODEM_SLEEP_MODE_NONE) {
        // Just register to sleep module, make the modem sleep modules check BT
        // sleep status when sleep enter. Thus, it will prevent WIFI from
        // disabling RF when BT is not in sleep but is using RF.
    ...
    if (ret) {
        ...
        } else if (btdm_controller_get_sleep_mode() ==
                   BTDM_MODEM_SLEEP_MODE_EVED) {
            esp_modem_sleep_deregister(MODEM_BLE_MODULE);
        }
